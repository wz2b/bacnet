...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
func decode_character_string(
	apdu []byte,
	lenValue uint32,
) (int, BACNET_CHARACTER_STRING) {
	charString := BACNET_CHARACTER_STRING{
	...
	stringLength := lenValue - 1

	status := characterstring_init(
		&charString,
		apdu[0],
		apdu[1:1+int(stringLength)],
		stringLength,
	)
	if !status {
		return 0, charString
	...
	return int(lenValue), charString
}
